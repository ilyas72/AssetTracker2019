...
if(isset($_POST['Submit'])) {	
	...
	$name = $_POST['name'];
	$netbook_name = $_POST['netbook_name'];
	...
	$staff_id = $_SESSION['staff_id'];
			...
			if($netbook_name) {
		foreach ($netbook_name as $c) {
			mysqli_query($db, "INSERT INTO loan_netbook_tbl(netbook_name, staff_id, name, date_from, time_from, date_to, time_to) VALUES 
...
if(isset($_POST['Submit'])) {	
	...
	$staff_id = $_SESSION['staff_id'];
			...
			if($netbook_name) {
		foreach ($netbook_name as $c) {
			mysqli_query($db, "INSERT INTO record_netbook_tbl(netbook_name, staff_id, name, date_from, time_from, date_to, time_to) VALUES 
			('".mysqli_real_escape_string($db, $c)."', ".$staff_id." ,'$name','$date_from','$time_from','$date_to','$time_to')");
			  if(send)
		//display success message
		echo '<script type="text/javascript"> alert("Data added successfully!");
  ...
  for (var x=0; x<numFlds; x++) {
    for (var y=x+1; y<numFlds; y++) {
      if (document.myform.MyField[x].value == document.myform.MyField[y].value) {
        alert('netbook has been LOANs...! \nPlease choose another netbook.');
        return false;
      }
    }
  }
  alert('netbook Available! \nPlease Proceed.');
  return true;
}
</script>
</head>
...
<?php include("header.php"); ?>	
     <p><font size="+2">New Loan netbook</font></p>
    <br/>    
...
//query
	if($key !="")
		$sql_sel=mysql_query("SElECT * FROM loan_netbook_tbl");
	else
		 $sql_sel=mysql_query("SElECT * FROM loan_netbook_tbl");
    $i=0;
    ...
    ?>
   <input type="text" id="MyField" value="<?php echo $row['netbook_name'];?>" style="text-align:center; width:90px; height:100; background-color:#FF0004;"/>
    <?php	
    }
    ?></div></td>
      </tr>
    ...
    <td align="center">&nbsp;</td></tr>
      <tr> 
    <td align="center">netbook has been Loan</td></tr>
     <tr> 
    <td align="center">&nbsp;</td></tr>
		</table>
        <hr>
   		<table border="0" cellpadding="2" cellspacing="3">
            ...
            <tr> 
				<td>netbook no.</td>
				<td><select name="netbook_name[]" id="MyField" multiple="multiple" onClick="return testDupes()" required='required'/>
                <?php
			  //db connection
include("connections/connection.php");
//query
	          $query = "select * from  netbook_tbl";
               $results = mysql_query($query);
               while ($rows = mysql_fetch_assoc(@$results))
           {?>
        <option value="<?php echo $rows['netbook_name'];?>" ><?php echo $rows['netbook_name'];?></option>
    <?php
    }?></select>
     		</td>  </tr>
			<tr> 
