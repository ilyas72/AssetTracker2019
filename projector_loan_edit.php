...
{	
	$loan_netbook_id = $_POST['loan_netbook_id'];
	...
	if(empty($name) || empty($netbook_name) || empty($date_from) || empty($time_from) || empty($date_to) || empty($time_to)) {
				
		if(empty($name)) {
			echo "<font color='red'>Name field is empty.</font><br/>";
		}
		
		if(empty($netbook_name)) {
			echo "<font color='red'>Name field is empty.</font><br/>";
		}
		...
		//updating the table
		$result = mysql_query("UPDATE loan_netbook_tbl SET name='$name', netbook_name='$netbook_name', date_from='$date_from', time_from='$time_from', date_to='$date_to', time_to='$time_to' WHERE loan_netbook_id=$loan_netbook_id");
...
<?php
//getting id from url
$loan_netbook_id = $_GET['loan_netbook_id'];

//selecting data associated with this particular id
$result = mysql_query("SELECT * FROM loan_netbook_tbl WHERE loan_netbook_id=$loan_netbook_id");

while($res = mysql_fetch_array($result))
{
	$name = $res['name'];
	$netbook_name = $res['netbook_name'];
...
<?php include("header.php"); ?>	
     <p><font size="+2">Edit/Update netbook</font></p>
    <br/>  
    ...
    <center>
		<table border="0" cellpadding="2" cellspacing="3" width="400">
            ...
            <tr> 
				<td>netbook no.</td>
				<td><input type="text" name="netbook_name" value=<?php echo $netbook_name;?> readonly></td>
			</tr>
            <tr> 
				<td>Date From</td>
			...
			<tr>
				<td><input type="hidden" name="loan_netbook_id" value=<?php echo $_GET['loan_netbook_id'];?>></td>
