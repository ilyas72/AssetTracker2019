...
{	
	...
	if(empty($name) || empty($email) || empty($username) || empty($date_reg)) {
		...
		//updating the table
		$result = mysql_query("UPDATE staff_tbl SET name='$name', email='$email', username='$username', date_reg='$date_reg' WHERE staff_id=$staff_id");
...
$result = mysql_query("SELECT * FROM staff_tbl WHERE staff_id=$staff_id");
...
<center>
  ...
  <p><font size="+2">Edit/Update Staff</font></p>
    <br/> 
	<form name="form1" method="post" action="edit_staff.php">
		<table border="0">
			<tr> 
				<td>Name</td>
			...
			<tr>
				<td><input type="hidden" name="staff_id" value=<?php echo $_GET['staff_id'];?>></td>
